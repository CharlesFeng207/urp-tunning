Shader "Hidden/Universal Render Pipeline/BloomCustom"
{
    Properties
    {
    ...
    HLSLINCLUDE
        ...
        #if _WITH_BLUR
            half dd = max(abs(uv.x - 0.5), abs(uv.y - 0.5));
            return dd > _BloomWithBlurStartRatio ? color : color * multiplier;
        #else
        ...
        {
            UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);
            float texelSize = _MainTex_TexelSize.x * _BloomInterIndex * Scatter;
            float2 uv = UnityStereoTransformScreenSpaceTex(input.uv);
        ...
        {
            UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);
            float texelSize = _MainTex_TexelSize.y * _BloomInterIndex * Scatter * 0.5;
            float2 uv = UnityStereoTransformScreenSpaceTex(input.uv);
